...
if [ $? -ne 0 ]; then
    log_error "failed to copy antchain-bridge-commons-${SDK_VERSION}.jar"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    log_error "failed to copy antchain-bridge-spi-${SDK_VERSION}.jar"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    log_error "failed to copy antchain-bridge-plugin-lib-${SDK_VERSION}.jar"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    log_error "failed to copy antchain-bridge-plugin-manager-${SDK_VERSION}.jar"
    exit 1
fi
