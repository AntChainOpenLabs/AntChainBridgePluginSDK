...
import com.alipay.antchain.bridge.plugins.manager.pf4j.asm.BBCServiceInfo;
import org.pf4j.*;
...
public abstract class AbstractExtensionFinder implements ExtensionFinder, PluginStateListener {
    ...
    protected volatile Map<String, BBCServiceInfo> extensionInfos; // cache extension infos by class name
        ...
        for (String className : classNames) {
            try {
                if (isCheckForExtensionDependencies()) {
                    ...
                    BBCServiceInfo extensionInfo = getExtensionInfo(className, classLoader);
...
//                    }
     ...
     */
    private BBCServiceInfo getExtensionInfo(String className, ClassLoader classLoader) {
        ...
        if (!extensionInfos.containsKey(className)) {
            ...
            BBCServiceInfo info = BBCServiceInfo.load(className, classLoader);
            if (info == null) {
