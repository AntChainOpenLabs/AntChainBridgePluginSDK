...
library SDPLib {
    ...
    function getSendingSeqID(string memory receiveDomain, identity sender, bytes32 receiver) pure internal returns (bytes32) {
    ...
    function encodeCrossChainIDIntoAddress(bytes32 id) pure internal returns (identity) {
        bytes memory rawId = new bytes(32);
        TypesToBytes.bytes32ToBytes(32, id, rawId);
        return BytesToTypes.bytesToAddress(32, rawId);
    }

    function getReceiverAddress(SDPMessageV2 memory sdpMessage) pure internal returns (identity) {
        return encodeCrossChainIDIntoAddress(sdpMessage.receiver);
    }

    function getReceiverAddress(SDPMessage memory sdpMessage) pure internal returns (identity) {
        return encodeCrossChainIDIntoAddress(sdpMessage.receiver);
    }

    function getReceiverAddress(SDPMessageV3 memory sdpMessage) pure internal returns (identity) {
        return encodeCrossChainIDIntoAddress(sdpMessage.receiver);
    }
