...
library TypesToBytes {
    ...
    function addressToBytes(uint _offset, identity _input, bytes memory _output) internal pure {
    ...
    function addressToBytes32(identity _input) internal pure returns (bytes32 _output) {
    ...
    function uint256ToBytes(uint _offset, uint256 _input, bytes memory _output) internal pure {

        assembly {
            let my_pos := add(_output, _offset)
            let prev_word_pos := sub(my_pos, 32)
            let prev_word := mload(prev_word_pos)
