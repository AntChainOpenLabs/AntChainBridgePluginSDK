...
public class TLVUtils {
            ...
            for (Field field : orderFields(fieldList, maxOrder, requiredOrderList)) {
                ...
                if (
                    ...
                    if (ObjectUtil.isEmpty(mapping)) {
                        ...
                        } else if (tlvField.type() == TLVTypeEnum.VAR_INT && BigInteger.class.isAssignableFrom(fieldClz)) {
                            items.add(getItem(tlvField.type(), tlvField.tag(), value));
                        } else {
                            throw new AntChainBridgeCommonsException(
                                    CommonsErrorCodeEnum.CODEC_TLV_ENCODE_ERROR,
                                    String.format(
                                            "field class %s has no annotation @TLVMapping must be TLV type BYTES or BYTES_ARRAY or VAR_INT",
                                            fieldClz.getName()
                                    )
                            );
                        }
                    ...
                    if (
                        ...
                        if (ObjectUtil.isEmpty(mapping)) {
                            if (StrUtil.equals(byte[].class.getTypeName(), val.getClass().getTypeName())) {
                                ...
                                Class elementClz = clz.getClassLoader().loadClass(((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0].getTypeName());
                                List<Object> fieldVal = new ArrayList<>();
                                for (Object o : (List) val) {
                                    fieldVal.add(decode((byte[]) o, elementClz));
                                }
                                field.set(obj, fieldVal);
                            } else if (BigInteger.class.isAssignableFrom(fieldClz)) {
                                field.set(obj, val);
                            } else {
                                throw new AntChainBridgeCommonsException(
                                        CommonsErrorCodeEnum.CODEC_TLV_DECODE_ERROR,
    ...
    private static Object objectFromCreator(Field field, Object val) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        for (Method method : field.getType().getMethods()) {
            ...
            if (!ClassUtil.isAssignable(method.getParameterTypes()[0], val.getClass())) {
                continue;
            }
    ...
    private static Object parseValueFromItem(TLVItem item, TLVTypeEnum type, Type fieldGenericType) {
        switch (type) {
            ...
            case MAP:
                return item.getMap(
                        (Class) TypeUtil.getTypeArgument(fieldGenericType, 0),
                        (Class) TypeUtil.getTypeArgument(fieldGenericType, 1)
                );
            case VAR_INT:
                if (fieldGenericType == long.class || fieldGenericType == Long.class) {
                    return new BigInteger(item.getValue()).longValue();
                } else if (fieldGenericType == int.class || fieldGenericType == Integer.class) {
                    return new BigInteger(item.getValue()).intValue();
                } else if (fieldGenericType == BigInteger.class) {
                    return new BigInteger(item.getValue());
                }
                throw new AntChainBridgeCommonsException(
                        CommonsErrorCodeEnum.CODEC_TLV_DECODE_ERROR,
                        "var int type doesn't support: " + type
                );
            default:
                throw new AntChainBridgeCommonsException(
                        CommonsErrorCodeEnum.CODEC_TLV_DECODE_ERROR,
        ...
        switch (type) {
            case UINT8:
                if (value instanceof Integer) {
                    ...
                    item = TLVItem.fromUint8(tag, (Boolean) value ? (byte) 1 : (byte) 0);
                } else {
            ...
            case VAR_INT:
                ...
                if (value.getClass() == long.class || value instanceof Long) {
                    rawValue = BigInteger.valueOf((long) value).toByteArray();
                } else if (value == int.class || value instanceof Integer) {
                    rawValue = BigInteger.valueOf((int) value).toByteArray();
                } else if (value instanceof BigInteger) {
                    rawValue = ((BigInteger) value).toByteArray();
                } else {
                    throw new RuntimeException("unsupported type for var int: " + value.getClass().getName());
                }
                item = TLVItem.fromBytes(tag, rawValue);
                break;
            default:
                throw new AntChainBridgeCommonsException(
