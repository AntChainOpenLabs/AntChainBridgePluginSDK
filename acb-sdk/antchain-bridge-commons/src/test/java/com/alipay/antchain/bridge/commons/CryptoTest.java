...
public class CryptoTest {
    ...
    public void testOid() {
        for (SignAlgoEnum signAlgo : SignAlgoEnum.values()) {
            log.info("testOid {}", signAlgo.getName());
            System.out.println("testOid " + signAlgo.getName());
            byte[] rawPubkey = signAlgo.getSigner().generateKeyPair().getPublic().getEncoded();
            X509PubkeyInfoObjectIdentity x509PubkeyInfoObjectIdentity = new X509PubkeyInfoObjectIdentity(rawPubkey);
            Assert.assertTrue(
                    ArrayUtil.equals(
                            rawPubkey,
                            x509PubkeyInfoObjectIdentity.getPublicKey().getEncoded()
                    )
            );
        }
    }
