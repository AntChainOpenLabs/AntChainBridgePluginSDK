...
public class Pf4jAntChainBridgePlugin extends AbstractAntChainBridgePlugin {
    ...
    public void start() {
        ...
        this.pf4jPluginManager.getExtensionClasses(IHeteroChainDataVerifierService.class, this.pluginWrapper.getPluginId()).forEach(
                aClass -> this.setHcdvsServiceFactory(new Pf4jHCDVSServiceFactory(aClass))
        );
    ...
    @Override
    @Synchronized
    public IHeteroChainDataVerifierService createHCDVSService() {
        if (ObjectUtil.notEqual(this.getState(), AntChainBridgePluginState.START)) {
            throw new AntChainBridgePluginManagerException(
        ...
        return this.getHcdvsServiceFactory().create();
    }

    @Override
    @Synchronized
    public IHeteroChainDataVerifierService createHCDVSService(Logger logger) {
        if (ObjectUtil.notEqual(this.getState(), AntChainBridgePluginState.START)) {
            throw new AntChainBridgePluginManagerException(
                    AntChainBridgePluginManagerErrorCodeEnum.CREATE_HCDVSSERVICE_FAILED,
                    String.format("plugin is %s", this.getState().name())
        ...
        return this.getHcdvsServiceFactory().create(logger);
    }
