...
import com.alipay.antchain.bridge.plugins.manager.pf4j.asm.AsmUtils;
import com.alipay.antchain.bridge.plugins.manager.pf4j.asm.IAntChainBridgeServiceInfo;
import org.pf4j.*;
...
public abstract class AbstractExtensionFinder implements ExtensionFinder, PluginStateListener {
    ...
    protected volatile Map<String, IAntChainBridgeServiceInfo> extensionInfos; // cache extension infos by class name
    ...
    @Override
    @SuppressWarnings("unchecked")
    public <T> List<ExtensionWrapper<T>> find(Class<T> type, String pluginId) { // 目前类型被写死了，只能BBCServiceInfo
        ...
        for (String className : classNames) {
            try {
                if (isCheckForExtensionDependencies()) {
                    ...
                    if (getExtensionInfo(className, classLoader) == null) {
...
//                    for (String requiredPluginId : extensionInfo.getProducts()) {
...
//                    }
     ...
     */
    private IAntChainBridgeServiceInfo getExtensionInfo(String className, ClassLoader classLoader) {
        ...
        if (!extensionInfos.containsKey(className)) {
            ...
            IAntChainBridgeServiceInfo info = AsmUtils.load(className, classLoader);
            if (info == null) {
