...
public class Pf4jAntChainBridgePluginManager implements IAntChainBridgePluginManager {
    ...
    private final Map<String, IHeteroChainDataVerifierService> hcdvsServiceMap = MapUtil.newConcurrentHashMap();
    ...
    public IHeteroChainDataVerifierService createHCDVSService(String product) {
        if (!hasPlugin(product)) {
            throw new AntChainBridgePluginManagerException(
                    AntChainBridgePluginManagerErrorCodeEnum.NONE_PLUGIN_FOUND_FOR_BLOCKCHAIN_PRODUCT, // yuechi: 这里要加一个验证的ErrorCodeEnum
                    String.format("No plugin for blockchain %s", product) // yuechi: 这里异常报错要改
            );
        }

        IHeteroChainDataVerifierService service = this.getPlugin(product).createHCDVSService();
        this.hcdvsServiceMap.put(product, service);

        return service;
    }

    @Override
    public IHeteroChainDataVerifierService createHCDVSService(String product, Logger logger) {
        if (!hasPlugin(product)) {
            throw new AntChainBridgePluginManagerException(
                    AntChainBridgePluginManagerErrorCodeEnum.NONE_PLUGIN_FOUND_FOR_BLOCKCHAIN_PRODUCT, // yuechi: 同理
                    String.format("No plugin for blockchain %s", product) // yuechi: 同理
            );
        }

        IHeteroChainDataVerifierService service = this.getPlugin(product).createHCDVSService(logger);
        this.hcdvsServiceMap.put(product, service);
    ...
    public IHeteroChainDataVerifierService getHCDVSService(String product) {
        if (!hasPlugin(product)) {
            throw new AntChainBridgePluginManagerException(
                    AntChainBridgePluginManagerErrorCodeEnum.NONE_PLUGIN_FOUND_FOR_BLOCKCHAIN_PRODUCT, // yuechi: 同理
                    String.format("No plugin for blockchain", product) // yuechi: 同理
            );
        }

        if (this.hcdvsServiceMap.containsKey(product)) {
            return this.hcdvsServiceMap.get(product);
        }
        return null;
    }
    ...
    public boolean hasProduct(String product) {
        return this.hcdvsServiceMap.containsKey(product);
    }
