    ...
    public void registerThirdPartyBlockchainTrustAnchor(RegisterThirdPartyBlockchainTrustAnchorRequest request) throws AntChainBridgeBCDNSException {
        Response response = stub.registerThirdPartyBlockchainTrustAnchor(
                RegisterThirdPartyBlockchainTrustAnchorReq.newBuilder()
                        ...
                        .setTpbta(ByteString.copyFrom(request.getTpbta().encode()))
                        .build()
        );
        if (response.getCode() != 0) {
    ...
    public ThirdPartyBlockchainTrustAnchor queryThirdPartyBlockchainTrustAnchor(QueryThirdPartyBlockchainTrustAnchorRequest request) {
        Response response = stub.queryThirdPartyBlockchainTrustAnchor(
                QueryThirdPartyBlockchainTrustAnchorReq.newBuilder()
                        .setTpbtaLaneKey(request.getTpBtaCrossChainLane().getLaneKey())
                        .setTpbtaVersion(request.getTpbtaVersion())
                        .build()
        );
        if (response.getCode() != 0) {
        ...
        return ThirdPartyBlockchainTrustAnchor.decode(response.getQueryThirdPartyBlockchainTrustAnchorResp().getTpbta().toByteArray());
    }

    @Override
    public PTCTrustRoot queryPTCTrustRoot(ObjectIdentity ptcOid) {
        Response response = stub.queryPtcTrustRoot(
                QueryPtcTrustRootReq.newBuilder()
                        .setPtcOid(ByteString.copyFrom(ptcOid.encode()))
                        .build()
        );
        if (response.getCode() != 0) {
            throw new AntChainBridgeBCDNSException(BCDNSErrorCodeEnum.BCDNS_QUERY_PTCTRUSTROOT_FAILED, response.getErrorMsg());
        }
        if (!response.hasQueryPtcTrustRootResp()) {
            throw new AntChainBridgeBCDNSException(
                    BCDNSErrorCodeEnum.BCDNS_QUERY_PTCTRUSTROOT_FAILED,
                    "response has no QueryThirdPartyBlockchainTrustAnchorResp"
            );
        }
        return PTCTrustRoot.decode(response.getQueryPtcTrustRootResp().getPtcTrustRoot().toByteArray());
    }

    @Override
    public void addPTCTrustRoot(PTCTrustRoot ptcTrustRoot) {
        Response response = stub.addPTCTrustRoot(
                AddPTCTrustRootReq.newBuilder()
                        .setPtcTrustRoot(ByteString.copyFrom(ptcTrustRoot.encode()))
                        .build()
        );
        if (response.getCode() != 0) {
            throw new AntChainBridgeBCDNSException(BCDNSErrorCodeEnum.BCDNS_SET_PTCTRUSTROOT_FAILED, response.getErrorMsg());
        }
    }
